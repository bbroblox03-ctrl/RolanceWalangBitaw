...
--// Blacklist helper — skip broken NPC/Dialogue paths that throw bytecode errors
local BLACKLIST_NAMES = {NPC = true, Dialogue = true, Other = true, ["Influencer Collabs"] = true}
local function isBlacklisted(obj)
    local p = obj
    while p do
        if BLACKLIST_NAMES[p.Name] then
            return true
        end
        p = p.Parent
    end
...
local function buyAllAffordable()
    for _, obj in ipairs(userTycoon.Purchases:GetDescendants()) do
        if not isBlacklisted(obj) and obj:IsA("Model") then
...
local function refreshUpgradeRemotes()
    ...
    for _, obj in ipairs(purchases:GetDescendants()) do
        if not isBlacklisted(obj) and obj:IsA("RemoteFunction") and obj.Name == "Upgrade" then
...
local RebirthGainMultiple = 35.0
