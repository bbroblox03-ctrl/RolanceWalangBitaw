...
local function getFlagValue(name, default)
    if not Rayfield or not Rayfield.Flags then return default end
    local f = Rayfield.Flags[name]
    if not f or type(f) ~= "table" then return default end
    if f.CurrentValue ~= nil then
...
local Paragraph = SettingsTab:CreateParagraph({
    Title = "How to use",
    Content = "Auto Roll uses requestRoll remote. Fast Roll spams with no delay. Auto Potion uses requestUseBoost remote every 2 seconds.\n\nAuto Save: All toggle states, slider values, dropdown selections, color pickers, and keybinds are automatically saved to your configuration file."
})

-- ==================== CONFIGURATION AUTO-LOAD ====================
task.spawn(function()
    -- Wait for Rayfield to finish loading and flags to be populated
    task.wait(2)

    -- Apply saved WalkSpeed
    local savedWalkSpeed = getFlagValue("ws1", 16)
    if savedWalkSpeed then
        local hum = getHum()
        if hum then hum.WalkSpeed = savedWalkSpeed end
    end

    -- Apply saved Fruit Radius
    local savedFruitRadius = getFlagValue("fruitRadius1", 30)
    if savedFruitRadius then
        State.fruitCollectionRadius = savedFruitRadius
    end

    -- Apply saved Potion Selection
    local savedPotion = getFlagValue("potionSelect1", "Luck Boost")
    if savedPotion then
        State.selectedPotion = savedPotion
    end

    -- Apply saved Potion Amount
    local savedPotionAmount = getFlagValue("potionAmount1", 1)
    if savedPotionAmount then
        State.potionAmount = savedPotionAmount
    end

    -- Apply saved ESP Color
    local savedESPColor = getFlagValue("espColor1", Color3.fromRGB(0, 150, 255))
    if savedESPColor then
        State.espColor = savedESPColor
    end

    -- Apply saved ESP Mode
    local savedESPMode = getFlagValue("espMode1", {})
    if savedESPMode and type(savedESPMode) == "table" then
        State.espMode = savedESPMode
    end

    -- Apply saved Theme
    local savedTheme = getFlagValue("Dropdown1", "Ocean")
    if savedTheme and type(savedTheme) == "string" then
        pcall(function()
            Window.ModifyTheme(savedTheme)
        end)
    end

    -- Sync toggle states from loaded flags
    State.toggles.humanizer1 = getFlag("humanizer1")
    State.toggles.auto1 = getFlag("auto1")
    State.toggles.auto2 = getFlag("auto2")
    State.toggles.autoFruit1 = getFlag("autoFruit1")
    State.toggles.autoZone1 = getFlag("autoZone1")
    State.toggles.autoRoll1 = getFlag("autoRoll1")
    State.toggles.fastRoll1 = getFlag("fastRoll1")
    State.toggles.autoPotion1 = getFlag("autoPotion1")
    State.toggles.autoBlaster1 = getFlag("autoBlaster1")
    State.toggles.esp1 = getFlag("esp1")
    State.humanizer = State.toggles.humanizer1
    State.espEnabled = State.toggles.esp1

    print("[Kyypie Hub] Configuration loaded successfully!")

    Rayfield:Notify({
        Title = "Kyypie Hub",
        Content = "Configuration auto-loaded!",
        Duration = 3,
        Image = 4483362458,
    })
end)
...
plr.CharacterAdded:Connect(function()
    ...
    -- Re-apply saved walkspeed after respawn
    task.wait(0.5)
    local savedWalkSpeed = getFlagValue("ws1", 16)
    if savedWalkSpeed then
        local hum = getHum()
